...
# Specify your Canarytoken Deploy Flock API Key
...
response = requests.post(
    f"{base_url}/api/v1/canarytoken/create", data=request_parameters, timeout=30
)
...
response = requests.get(
    f"{base_url}/api/v1/canarytoken/download",
...
token_aws_secret_access_key = re.findall(
    "aws_secret_access_key=(.*)", response_content
)[0]
...
if GITHUB_PERSONAL_ACCESS_TOKEN != "":
    ...
    response = requests.get(WORD_TEMPLATE_URL, headers=request_parameters, timeout=30)
else:
    print("Fetching Template using Github public channel")
    response = requests.get(WORD_TEMPLATE_URL, timeout=30)
...
response = requests.post(
    f"{base_url}/api/v1/canarytoken/create",
...
with open(os.path.join(tmp_folder, "word", "document.xml"), "r", encoding="utf-8") as f:
    ...
    filedata2 = filedata1.replace(
        AWS_TOKEN_PLACEHOLDER_KEY, token_aws_secret_access_key
    )
...
os.chmod(
    os.path.join(TARGET_FOLDER, random_token_folder, random_token_sub_folder), 0o777
)
os.chmod(token_path, 0o777)

os.utime(
    os.path.join(TARGET_FOLDER, random_token_folder),
    (Modified_timestamp, Modified_timestamp),
)
os.utime(
    os.path.join(TARGET_FOLDER, random_token_folder, random_token_sub_folder),
    (Modified_timestamp, Modified_timestamp),
)
