...
curl -XGET "https://$CANARY_HASH.canary.tools/api/v1/incidents/outside_bird/search" \
  -d auth_token=$CANARY_TOKEN \
  -d node_ids=$BIRD_ID \
  -G | jq '.src_ips[] | .ip_address' | sed 's/\"//g' | sort > canary-ips-$(date +%Y-%m-%d).txt

#curl https://$CANARY_HASH.canary.tools/api/v1/incidents/outside_bird/search -d auth_token=$CANARY_TOKEN -d node_ids=000103c21e6928dd -G | jq '.src_ips[] | .ip_address' | sed 's/\"//g' | sort > canary-ips-$(date +%Y-%m-%d).txt
...
for line in $lines
do
    curl "https://api.greynoise.io/v2/noise/context/$line" -H "key: $GREYNOISE_TOKEN" -H "accept: application/json" | jq '.' | cat >> Recent-Canary-GreyNoise-Enterprise-Threat-Intel-Report-$(date +%Y-%m-%d).json
done
