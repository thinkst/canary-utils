# This script will use a local docx file,
...
$localTemplateFile = 'C:\Path\To\Your\LocalTemplate.docx' # Path to your local Word document template
...
$localTemplateFileName = Split-Path $localTemplateFile -Leaf
$localTemplateFileBin = [System.IO.File]::ReadAllBytes($localTemplateFile)
$encodingScheme = [System.Text.Encoding]::GetEncoding("iso-8859-1")
$encodedFile = $encodingScheme.GetString($localTemplateFileBin)
...
$word_FlockID
--$httpBoundary
Content-Disposition: form-data; name ="doc"; filename ="$localTemplateFileName"
...
Expand-Archive -path $zipName -DestinationPath $OutputFileName.Split('.')[0]

$xmlToEdit = $OutputFileName.Split('.')[0] + '\word\document.xml'
